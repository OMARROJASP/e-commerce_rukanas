...
const loadRoutes = async () => {
    const files = (0, fs_1.readdirSync)(__dirname).filter((file) => file !== "index.ts" && file !== "index.js");
    await Promise.all(files.map(async (file) => {
        const routeName = cleanFileName(file);
        if (!routeName)
            return;
        try {
            const module = await Promise.resolve(`${`./${routeName}`}`).then(s => __importStar(require(s)));
